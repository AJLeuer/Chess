...
/* Forward declarations: */
	...
	public:
		...
		inline virtual const vec2 <int> getMaxPosition () {
			return vec2 <int> {(int) boardRepresentation.size(), (int) boardRepresentation[0].size()};
		}
		...
		vector <const Square *>
		getSpecifiedSquares (const vec2 <int> startingSquarePosition, const vector <Direction> & directions,
 ...
 */
	...
	basic_ostream <Character> & operator << (basic_ostream <Character> & out, const Board & board) {
		...
		for (unsigned i = 0; i < ranks; i++) {
			...
			out << L"│ " << * (board.getSquare((unsigned) 0, i)) << L" │ " << * (board.getSquare((unsigned) 1, i))
			    << L" │ " << * (board.getSquare((unsigned) 2, i)) << L" │ " << * (board.getSquare((unsigned) 3, i))
			    << L" │ " << * (board.getSquare((unsigned) 4, i)) << L" │ " << * (board.getSquare((unsigned) 5, i))
			    << L" │ " << * (board.getSquare((unsigned) 6, i)) << L" │ " << * (board.getSquare((unsigned) 7, i))
			    << L" │" << endl;
