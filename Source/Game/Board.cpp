...
namespace Chess {
	...
	vector <const Square *>
	Board::getSpecifiedSquares (const vec2 <int> startingSquarePosition, const vector <Direction> & directions,
			...
			for (vec2 <int> next = (startingSquarePosition + offset);; next = (next + offset)) {
				...
				if (isInsideBoardBounds(next)) {
					...
					if (nextSquare->isOccupied()) { //the other way to get out of the loop
						...
						if ((includeFirstPieceOfColorEncountered == SafeBoolean::t) &&
						    (nextSquare->getPiece()->getColor() == toInclude)) {
	...
	vector <const Square *>
	Board::getSpecifiedSquares (const vec2 <int> startingSquarePosition, const vector <Direction> & directions,
	                            int maxSearchDistance, SafeBoolean includeFirstPieceOfColorEncountered,
	                            Color toInclude) const {
	...
	vector <const Square *>
	Board::getSpecifiedSquares (const vec2 <int> startingSquarePosition, int searchRadius) const {
