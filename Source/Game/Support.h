...
struct GameRecordEntry {
	...
	struct AlgrebraicNotation {
		...
		AlgrebraicNotation(const Piece & piece, const RankAndFile & destination) : pieceSymbol(piece.getSymbol()), destination(destination) {}
	...
	GameRecordEntry(const Piece & piece, const RankAndFile & destination) : algrebraicNotation(piece, destination), startingPosition(* piece.getPosition()) {}
